...
BEGIN
	IF NOT EXISTS (
		...
		WHERE conname = 'corsair_accounts_integration_id_corsair_integrations_id_fk'
	) THEN
...
BEGIN
	IF NOT EXISTS (
		SELECT 1 FROM pg_constraint
		WHERE conname = 'corsair_entities_account_id_corsair_accounts_id_fk'
	) THEN
...
BEGIN
	IF NOT EXISTS (
		SELECT 1 FROM pg_constraint
		WHERE conname = 'corsair_events_account_id_corsair_accounts_id_fk'
	) THEN
